...
class Settings(BaseSettings):
    ...
    META_OAUTH_REDIRECT_URI: str = "https://apps.orvym.com/dashboard/integrations/"  # Frontend spawn domain used for dashboard/verify display ONLY. IMPORTANT: For the Embedded Signup FB.login + config_id (Facebook Login for Business) popup flow the token exchange sends ONLY client_id + client_secret + code (NO redirect_uri) - per Meta's current official docs the code is returned directly to the JS popup callback (no server-side redirect). This canonical app URL is for display/verification only and is NEVER sent to Meta during the exchange. Do NOT send redirect_uri in the exchange; sending the JS SDK's internal xd_arbiter channel URL (https://staticxx.facebook.com/x/connect/xd_arbiter/?version=46) fails with Meta error code 191 ("The domain of this URL isn't included in the app's domains") because staticxx.facebook.com is a Meta-internal domain that must never be added to App Domains.
