...
redirect_uri="" - the empty string for the Embedded Signup FB.login popup
flow) and capture Meta's actual response.
...
async def run_exchange(svc: MetaOAuthService, code: str) -> None:
    ...
    print("  redirect_uri: EMPTY STRING (Embedded Signup FB.login popup flow)")
    ...
    if not success:
        ...
        print("  client_id + client_secret + code + redirect_uri='' for the")
