...
def test_exchange_36008_error_includes_actionable_hint():
    ...
    redirect_uri) and the exchange sends NO redirect_uri (the official
    Embedded Signup exchange sends client_id + client_secret + code only - a
    36008 means the code itself was consumed, expired or issued outside the
    config_id popup flow).
    """
    captured = {}
    svc = MetaOAuthService(app_id="3862862217342382", app_secret="secret")
    svc.GRAPH_API_BASE = GRAPH_BASE
