...
def test_exchange_36008_error_includes_actionable_hint():
    ...
    redirect_uri) and the exchange still sent redirect_uri='' (Embedded Signup
    flow - a 36008 with redirect_uri='' means the code itself was consumed,
    expired or issued outside the JS SDK popup flow).
    """
    captured = {}
    svc = MetaOAuthService(app_id="3862862217342382", app_secret="secret")
    svc.GRAPH_API_BASE = GRAPH_BASE
